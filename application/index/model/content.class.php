...
class content {
	...
	public static function content_page($content) {
		...
		if(URL_MODEL == 3){
			$pages .= $page<=1 ? '<li class="page-item"><a href="?page=1" class="page-link">'.L('pre_page').'</a>' : '<a href="?page='.($page-1).'" class="page-link">'.L('pre_page').'</a>';
			for($i=1; $i<=$total_page; $i++){  
				$class = $i==$page ? ' active' : '';
				$pages.='<li class="page-item '.$class.'"><a href="?page='.$i.'" class="page-link">'.$i.'</a></li>';
			}
			$pages .= $page>=$total_page ? '<li class="page-item"><a href="?page='.$page.'" class="page-link">'.L('next_page').'</a></li>' : '<li class="page-item"><a href="?page='.($page+1).'" class="page-link">'.L('next_page').'</a>';
		}else{
